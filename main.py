...
def video_cap(videos=0): # если нет аргумента - камера id 0
    ...
    if not cap.isOpened():
        ...
        vide = 'камера' if str(videos) == '0' else videos  # дублируется на всякий ....
        print(f'Видео запущено из источника: {vide} в {time.strftime('%H:%M:%S')} ')
        video_start(cap)
        return True
### регулировка яркости работает плоло.... так что пока не используем
...
#         target_brightness: Желаемая средняя яркость изображения (по умолчанию 127, середина диапазона 0-255).
#
#     Returns:
...
def video_start(cap):
    ...
    while True:
        ...
        for contour in contours:
            ...
            print(info)
            cv2.rectangle(frame1, (x, y), (x + w, y + h), (1, 255, 0), 1)
            #cv2.rectangle(adjusted_frame, (x, y), (x + w, y + h), (1, 255, 205), 2)
        ...
        if recording:
            ...
            # Конвертация - нужно переделывать архвацию на раз ~ в 7 дней и перенос в другую папку
            #     try:
            #         clip = VideoFileClip(video_filename)
            #         clip.write_videofile(f'{video_filename.replace('.mp4', '_conv.mp4')}',
            #                      codec="libx264")
            #         if os.path.exists(video_filename):
            #             os.remove(video_filename)
            #         clip.close()
            #
            #         print(f"\nВидео успешно конвертировано в {video_filename}_")
            #     except Exception as e:
            #         print(f"\nОшибка при конвертации видео: {e}")
            elif motion_found:
                motion_detected_time = time.time()
        #adjusted_frame = adjust_brightness(frame1)
